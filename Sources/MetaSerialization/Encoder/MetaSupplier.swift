...
public protocol MetaSupplier {
    ...
    /**
     ...
     - Attention: You should implement this method or keep the default implementation of it, even if you do not support keyed containers in you framework, because the documentation of Encodable specifies an empty keyed container as substitute if a value did not encode anything. Do not use precondition or assert in this method for that reason. Also keyed containers are an important part of many `encode(to:)` implementations (especially the ones generated by default). Therefor it could make sence to implement some transition from keyed containers to other containers you provide. A custom implementation of `EncodingKeyedContainerMeta` is the right place for this.
     */
    ...
    /**
     Creates, sets and returns a `Meta` for the given value, or returns nil if the value is not supported (can not be represented in your underlying format).
     
     This method will be asked to return a Meta for an instance of the type NilMarker if encoding a nil value is requested.
     ...
     You may not call Encodables `encode(to:)` in this method directly, neighter any of the container methods.
