...
public protocol Unwrapper {
    ...
    /**
     ...
     If you don't support the requested type directly (can't convert the Meta to T), return nil.
     
     If you decoded to a Meta conforming to NilMeta, that Meta will not reach your method.
     
     In this method, use the decoders container methods to request decoding nested values.
     However calling `decoder.decode(type:, from:)` is not allowed and will throw `MetaDecoder.Errors.decodingHasNotSucceeded`.
     Although not recommended, you may also directly call Decodables `init(from:)` in this method.
     ...
     This method is called frequently during deconding.
     ...
     - Returns: A value of type T that was constructed from meta. Returns nil, if the requested type is not supported directly or meta is not unwrappable to it.
     */
    ...
    /**
     ...
     Therefore, if meta does conform to `NilMeta` you do not need to handle it in this method.
